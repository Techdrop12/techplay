...
function drawInvoice(doc: PdfDoc, data: InvoiceData, opts: PdfRenderOptions) {
  ...
  data.items.forEach((item, index) => {
    ...
    doc.text(item.name || `Article ${index + 1}`, columns.title, y, { width: columns.qty - columns.title - 20 })
  ...
  if (data.discount?.amount) {
    rightRow(`Remise${data.discount.code ? ` (${data.discount.code})` : ''}`, `- ${money(data.discount.amount)}`)
  }
...
function getBrand(brand?: BrandInfo): BrandInfo {
  return {
    ...
    address: brand?.address || process.env.BRAND_ADDRESS || '42 rue de la Liberté\n75000 Paris\nFrance',
