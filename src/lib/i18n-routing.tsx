...
// Routes that live under app/[locale]/... — only these get a locale prefix.
const LOCALIZED_PATHNAMES: readonly string[] = [
  '/',
  '/products',
  '/wishlist',
  '/search',
...
function readCookie(name: string): string | undefined {
  ...
  try {
    const raw = document.cookie.split('; ').find((cookie) => cookie.startsWith(`${name}=`));
...
export function localizePath(path: string, locale: Locale, opts: LocalizeOptions = {}): string {
  ...
  const search = opts.customQuery ?? (parsed.search || (opts.keepQuery ? getCurrentSearch() : ''));

  const hash = opts.customHash ?? (parsed.hash || (opts.keepHash ? getCurrentHash() : ''));

  const localizedPathname = isLocalizedRoute(stripped) ? _withLocale(stripped, locale) : stripped;
