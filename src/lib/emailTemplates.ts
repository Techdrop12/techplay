...
const ADDRESS = process.env.NEXT_PUBLIC_COMPANY_ADDRESS ?? "TechPlay, 10 rue de l’Innovation, 75000 Paris, France";
...
function preheader(text: string) {
  // masqué mais présent dans les preview d’e-mails
...
function asText(html: string): string {
  // Conversion texte simple (sans dépendance) : on garde une lisibilité correcte.
  return html
...
// Bonus : reset password (prêt si tu en as besoin)
...
// Utilitaire pour rendre un template avec sécurité
