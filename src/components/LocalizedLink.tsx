...
function hrefToAbsoluteUrl(href: NextHref): string {
  ...
  if (typeof href === 'string') {
    if (href.startsWith('http') || href.startsWith('//') || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('#')) return href
    return new URL(href, origin).href
  }
  const path = href.pathname ?? '/'
  const q = href.query ? '?' + new URLSearchParams(href.query as Record<string, string>).toString() : ''
  const h = href.hash ?? ''
...
function isInternalLink(href: NextHref): boolean {
  if (typeof href === 'string') {
    ...
    if (/^https?:\/\//i.test(href) && typeof window !== 'undefined' && !href.startsWith(window.location.origin)) return false
    return true
  }
  return true
}
...
const LocalizedLink = forwardRef<HTMLAnchorElement, Props>(function LocalizedLink(
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    fallbackTimeoutRef.current = setTimeout(() => {
      fallbackTimeoutRef.current = null
      try {
        if (typeof window !== 'undefined' && targetUrlRef.current && window.location.href !== targetUrlRef.current) {
