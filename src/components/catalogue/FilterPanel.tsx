...
const Icon = {
  ...
  Mouse: (p: SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" {...p}>
      <path
        fill="currentColor"
  ...
  Drive: (p: SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" {...p}>
      <path
        fill="currentColor"
...
const CAT_ICONS: Record<string, IconComponent> = {
...
export default function FilterPanel({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const current = url.searchParams.get(syncQueryKey)?.trim() || '';
      if (!current) return;

      const matched = categories.find((cat) => cat.toLowerCase() === current.toLowerCase()) ?? null;
      if (matched && matched !== selected) {
        setSelected(matched);
      }
    } catch {}
  }, [categories, selected, setSelected, syncQueryKey]);
  ...
  const currentIndex = useMemo(() => {
    ...
    const index = items.findIndex((x) => x.toLowerCase() === selected.toLowerCase());
  ...
  const onKeyDown: KeyboardEventHandler<HTMLDivElement> = (e) => {
    ...
    else setSelected(items[target] ?? null);
