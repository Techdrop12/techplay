...
const RAW_LIMIT = Number(process.env.NEXT_PUBLIC_WISHLIST_LIMIT ?? 50);
const WISHLIST_LIMIT = Number.isFinite(RAW_LIMIT) && RAW_LIMIT > 0 ? Math.min(RAW_LIMIT, 500) : 50;
...
function writeWishlist(list: WishlistProduct[]) {
  ...
  try {
    ...
    // broadcast multi-onglets/composants
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.88 }}
        className={cn(floating ? baseFloating : baseInline, className)}
