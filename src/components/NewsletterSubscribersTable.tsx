...
export default function NewsletterSubscribersTable() {
  ...
  const fetchPage = useCallback(async (skip: number) => {
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/newsletter-subscribers?limit=${PAGE_SIZE}&skip=${skip}`)
      ...
      {items.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm" aria-label="Inscrits newsletter">
            ...
            <tbody>
              {items.map((s) => (
                <tr key={s._id} className="border-b border-[hsl(var(--border))] hover:bg-[hsl(var(--surface-2))]">
