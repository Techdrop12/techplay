...
type MediaItem = string | { src: string; kind?: 'image' | 'video'; poster?: string };
...
export default function ProductGallery({
  ...
  const [_origin, _setOrigin] = useState({ x: 50, y: 50 });
  ...
  const thumbBtnsRef = useRef<(HTMLButtonElement | null)[]>([]);
  ...
  useEffect(() => {
    if (lightboxOpen) {
      ...
      setTimeout(() => {
        (lightboxRef.current?.querySelector('[data-close]') as HTMLElement | null)?.focus?.();
      }, 0);
      // reset zoom/pan
      setZoom(1);
  ...
  const handleThumbClick = (i: number) => setIndex(i);

  // Roving tabIndex sur vignettes (flèches)
  const onThumbsKeyDown = (e: React.KeyboardEvent) => {
  ...
  const imgOnError = (i: number) => setImgErrors((m) => ({ ...m, [i]: true }));
