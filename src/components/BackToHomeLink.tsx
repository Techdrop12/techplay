...
export default function BackToHomeLink({ className = '', prefetch = false, variant = 'default', 'aria-label': ariaLabel }: Props) {
  const t = useTranslations('common')
  const label = t('back_to_home')

  const baseClass = 'inline-flex min-h-[48px] items-center justify-center gap-2 rounded-full px-6 py-2.5 text-[15px] font-semibold transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--accent))] focus-visible:ring-offset-2'
  const classes =
    variant === 'outline'
