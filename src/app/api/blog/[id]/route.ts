...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const slug =
    body?.slug != null ? String(body.slug).trim().toLowerCase().replace(/\s+/g, '-') : undefined;
  ...
  try {
    ...
    if (body?.description !== undefined)
      doc.description = body.description ? String(body.description) : undefined;
    if (body?.image !== undefined) doc.image = body.image ? String(body.image).trim() : undefined;
    if (body?.author !== undefined)
      doc.author = body.author ? String(body.author).trim() : undefined;
