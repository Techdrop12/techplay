  ...
  try {
    ...
    for (const row of items as ImportItem[]) {
      ...
      await Product.create({
        ...
        price,
        oldPrice: row.oldPrice != null && !Number.isNaN(Number(row.oldPrice)) ? Number(row.oldPrice) : undefined,
        stock: row.stock != null && !Number.isNaN(Number(row.stock)) ? Math.max(0, Number(row.stock)) : 0,
