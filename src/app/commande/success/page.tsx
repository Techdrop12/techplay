...
export default async function OrderSuccessPage({
  ...
  return (
    <main
      className="container-app mx-auto w-full max-w-2xl py-20 sm:py-28"
      role="main"
    ...
    >
      <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--surface))] p-8 text-center shadow-[var(--shadow-lg)] sm:p-10">
        ...
        <p className="mt-3 text-[15px] text-token-text/75">{t('email_sent')}</p>
        <p className="mt-1 text-[13px] text-token-text/60">{t('delivery_info')}</p>
        ...
        <div className="mt-6 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--surface))]/80 p-4 text-left">
          {mock ? (
            ...
            <p className="text-[13px] text-token-text/80">
              {t('reference')}{' '}
              <code className="font-mono text-xs" title={sess}>
                {sess.length > 12 ? `…${sess.slice(-12)}` : sess}
              </code>
            </p>
          ) : (
