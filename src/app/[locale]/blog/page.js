...
export default function BlogPage() {
  ...
  const tSeo = useTranslations('seo');   // pour la partie meta (cles dans "seo")
  const tBlog = useTranslations('blog'); // pour traductions spécifiques au blog
  ...
  // Titre & description SEO (on peut aussi les prendre depuis "seo.blog_title"/"seo.blog_description")
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {posts.length === 0 ? (
          ...
          <div className="grid gap-6">
            {posts.map((post) => (
              <BlogCard key={post._id} post={locale === 'en' && post.en ? { ...post, content: post.en } : post} />
            ))}
          </div>
        )}
      </div>
